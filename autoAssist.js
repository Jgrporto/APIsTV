...
const DEVICE_PHONE = "5524999162165"; // Seu numero conectado ao BotBot
const ADMIN_NUMBER = "5524999157259"; // Numero que pode mandar "ASSIST"
...
/* 4. FUNCAO QUE TRATA A MENSAGEM RECEBIDA DO CLIENTE */
async function onMensagem(cliente, mensagem) {
  ...
  if (cliente !== ADMIN_NUMBER) {
    console.log("Numero nao autorizado");
    return;
  }
  ...
  if (mensagem.trim().toUpperCase() === "ASSIST") {
    ...
    if (resposta.includes("jǭ solicitou")) {
      await enviarMensagem(cliente, "Este dispositivo ja gerou o teste hoje.");
      return;
    }
    ...
    if (!filtrado) {
      await enviarMensagem(cliente, "Nao encontrei dados do ASSIST PLUS.");
      return;
    }
...
/* SIMULACAO DE RECEBIMENTO DE MENSAGEM */
